...
using UnityEngine;
//
public class windFX_cyPIPES : MonoBehaviour {
	...
	public enum FXtype{
		ambientWind,
		rangedDirectionalWind,
		directionalWind
	}

	public FXtype windEffectType;

	[Header("Effect Parameters")]
	[Tooltip("Set the effective range of this effect")]
	public float range = 10.0f;
	[Tooltip("Hard intensity values are -3,-2,-1,0,1,2,3")]
	[Range(-3.0f,3.0f)]
	...
	void Update () {
		...
		if (windEffectType == FXtype.ambientWind) {
			//AMBIENT FX ON
			//AMBIENT FX ON
			//AMBIENT FX ON
			//AMBIENT FX ON
						...
						if (hits.Length > 0) {
							...
							if (tileHit != "") {	
								foreach (string data in dataRef.tileRecord.Keys) {
									...
									if (details [2] == "Fan") {//its a wind channel
										if (intensity > 0) {//if its a positive intensity
										...
										if (intensity < 0) {//if its a negative intensity
											tileHit = oppositeTile (tileHit);
											if (details [3] == tileHit) {//if its the tile
												foreach (string uID in dataRef.parser.allUnitObjects.Keys) {//lets communicate the ON command now
													if (uID == details [0]) {//if the unitID matches unitID
			...
			if (activeChs.Count > 0) {
				...
				//in intensity is set to 0 condition
				if (intensity == 0) {
					//change unit channel properties to 0 and clear local active list (ONLY ONCE SEND OFF TYPE COMMANDS)
					foreach (string data in activeChs) {
						//reminder data format is unitID,ch#,chType,tileAssignment
			...
			if (cyPIPESLogActive) {
				...
				if (intensity > 0) {
					Debug.DrawRay (gameObject.transform.position, endPoint, Color.cyan, 0.11f);
				}
				if (intensity < 0) {
					Debug.DrawRay (gameObject.transform.position, endPoint * -1, Color.cyan, 0.11f);
					Debug.DrawRay (gameObject.transform.position, endPoint, Color.gray, 0.11f);
				}
				//Debug.DrawRay (this.gameObject.transform.position, dataRef.userPos, Color.cyan,0.11f);
			}
		...
		//RANGED DIRECTIONAL FX ON
		//RANGED DIRECTIONAL FX ON
		if (windEffectType == FXtype.rangedDirectionalWind) {
			//If user is in active space
			if (dataRef.userActive) {
				if (intensity != 0) {//if the intensity is not 0
					...
					if (intensity > 0){
						endPoint = (userToEFdir + (gameObject.transform.forward.normalized * range));//endpoint direction from user
					}
					if (intensity < 0) {
						endPoint = (userToEFdir + (gameObject.transform.forward.normalized * range * -1));//endpoint direction from user if intensity is negative
					}
					RaycastHit fxhit;
					...
					if (cyPIPESErrorsActive) {
						Debug.DrawRay (dataRef.userPos, endPoint, Color.magenta, 0.11f);
					} //Magenta raycast annotation from user to directional endpoint
					...
					try {
						...
						if (fxhit.collider != null) {
							if (cyPIPESErrorsActive) {
								Debug.LogError ("cyPIPES ERROR: Tile hit name is unrecognized for use in " + gameObject.name + " as " + fxhit.transform.gameObject.name);
							}
						}
					}
					...
					if (directionalTile != null) {
						...
						if (userEFdistance <= range) {
							...
							if (hits.Length > 0) {
								...
								//Debug.Log ("CURRENT TILE HIT = " + tileHit);
								if (intensity > 0) {//if intensity is positive
									if (tileHit != "") {
										foreach (string data in dataRef.tileRecord.Keys) {
											//if FX type is matched on tile change unit channel propetries to intesnsity level and record unitID,ch in local active list (EVERY FRAME SEND ON TYPE COMMANDS)
											//reminder data format is unitID,ch#,chType,tileAssignment
								...
								if (intensity < 0) {//if intensity is negative
									tileHit = oppositeTile (tileHit);
									if (tileHit != "") {
										foreach (string data in dataRef.tileRecord.Keys) {
											//if FX type is matched on tile change unit channel propetries to intesnsity level and record unitID,ch in local active list (EVERY FRAME SEND ON TYPE COMMANDS)
			...
			if (activeChs.Count > 0) {
				...
				//if intensity is set to 0 condition
				if (intensity == 0) {
					//change unit channel properties to 0 and clear local active list (ONLY ONCE SEND OFF TYPE COMMANDS)
					foreach (string data in activeChs) {
						//reminder data format is unitID,ch#,chType,tileAssignment
			...
			if (cyPIPESLogActive) {
				...
				if (intensity > 0) {
					Debug.DrawRay (gameObject.transform.position, directionEndPoint, Color.cyan, 0.11f);
				}
				if (intensity < 0) {
					Debug.DrawRay (gameObject.transform.position, directionEndPoint*-1, Color.cyan, 0.11f);
				}
				//Debug.DrawRay (this.gameObject.transform.position, dataRef.userPos, Color.cyan,0.11f);
			}
		...
		//DIRECTIONAL FX ON
		//DIRECTIONAL FX ON
		if (windEffectType == FXtype.directionalWind) {
			//If user is in active space
			if (dataRef.userActive) {
				if (intensity != 0) {
					//Ditermine which tile is active for this effect (This is too basic. In the future you need analyze where on the tile it hits and include nearest 1 or two tiles for more of a gradient effect) Corners of the tile cude cause switching instead of gradient of effects
					...
					Vector3 endPoint = new Vector3();
					if (intensity > 0){
						endPoint = (userToEFdir + (gameObject.transform.forward.normalized * 1000));//endpoint direction from user
					}
					if (intensity < 0) {
						endPoint = (userToEFdir + (gameObject.transform.forward.normalized * 1000 * -1));//endpoint direction from user if intensity is negative
					}
					RaycastHit fxhit;
					...
					if (directionalTile != null) {
						...
						if (hits.Length > 0) {
							...
							if (intensity > 0) {//if intensity is positive
								...
								if (tileHit != "") {	
									foreach (string data in dataRef.tileRecord.Keys) {
										...
										string[] details = data.Split (',');
										if (details [2] == "Fan") {//its a wind channel
											if (details [3] == tileHit) {//if record includes the directional tile
												foreach (string uID in dataRef.parser.allUnitObjects.Keys) {//lets communicate the ON command now
													if (uID == details [0]) {//if the unitID matches unitID
														//change parameter in the correct channel
														if (details [1] == "ch1") {dataRef.parser.allUnitObjects [uID].GetComponent<cyPIPES> ().ch1 = intensity;}
							...
							if (intensity < 0) {//negative intensity
								tileHit = oppositeTile (tileHit);
								//Debug.Log ("CURRENT TILE HIT = " + tileHit);
								if (tileHit != "") {	
									foreach (string data in dataRef.tileRecord.Keys) {
	...
	void OnDrawGizmos() {
		...
		Gizmos.DrawIcon (transform.position, "Wind.png", true);
	}
	...
	int convertIntensity (int inputIntensity){
		if (inputIntensity >= 3) {
			return(1);
		}
		if (inputIntensity == 2) {
	...
	public void updateIntensity (int newIntensity){
		intensity = convertIntensity (newIntensity);
	}
