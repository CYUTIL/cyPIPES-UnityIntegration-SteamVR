...
public class heatFX_cyPIPES : MonoBehaviour {
	...
	public enum FXtype{
		ambientHeat,
		rangedDirectionalHeat,
		directionalHeat
	}

	public FXtype heatEffectType;

	[Header("Effect Parameters")]
	[Tooltip("Set the effective range of this effect")]
	public float range = 10.0f;
	//Implement this for directional heat and use directional light as sun;
	//public bool sunlight;
	//public GameObject sun;
	//Intensity not public for heat vEFDs in cyPIPES yet. Please keep it at 1 until cyPIPES hardware can support voltage fading safely.
	int intensity = 1;
	...
	void Update () {
		...
		if (heatEffectType == FXtype.ambientHeat) {
			//AMBIENT FX ON
			//AMBIENT FX ON
			//AMBIENT FX ON
			//AMBIENT FX ON
					...
					if (hits.Length > 0) {
						...
						if (tileHit != "") {	
							foreach (string data in dataRef.tileRecord.Keys) {
								...
								if (details [2] == "Heat") {//its a heat channel
			...
			if (activeChs.Count > 0) {
				...
				if (dataRef.userActive == false) {
					Debug.Log ("VR Headset outside of tracked space, no cy.PIPES heat effects are being communicated until headset is back within tracked space.");
			...
			if (cyPIPESLogActive) {
				...
				Debug.DrawRay (gameObject.transform.position, endPoint, Color.yellow, 0.11f);
		...
		//RANGED DIRECTIONAL FX ON
		//RANGED DIRECTIONAL FX ON
		if (heatEffectType == FXtype.rangedDirectionalHeat) {
			//If user is in active space
			if (dataRef.userActive) {
			...
			if (cyPIPESLogActive) {
				...
				Debug.DrawRay (gameObject.transform.position, directionEndPoint, Color.yellow, 0.11f);
		...
		//DIRECTIONAL FX ON
		//DIRECTIONAL FX ON
		if (heatEffectType == FXtype.directionalHeat) {
			//If user is in active space
			if (dataRef.userActive) {
				//Ditermine which tile is active for this effect (This is too basic. In the future you need analyze where on the tile it hits and include nearest 1 or two tiles for more of a gradient effect) Corners of the tile cude cause switching instead of gradient of effects
				Vector3 thisFXdirection = gameObject.transform.forward;//EF direction
	...
	void OnDrawGizmos() {
		...
		Gizmos.DrawIcon (transform.position, "Heat.png", true);
	}
